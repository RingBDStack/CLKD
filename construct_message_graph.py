...
def construct_incremental_dataset_0922(args, df, save_path, features,nfeatures, test=False):
    ...
    if args.is_static:
        if args.lang == "Mix":
            ini_df = df.loc[df['date'].isin(distinct_dates[::])]
        else:
    ...
    if nfeatures is not None:
        ...
        print("trans features saved at 0!")
...
def main(args):
    if args.is_static:
        save_path = "./803_hash_static-{}-{}/".format(str(args.days),args.lang)
    else:
        save_path = "./0803_ALL_{}/".format(args.lang)
    if args.lang == "Mix":
        save_path = "0803-mix-ALL/"
        filename = "/data/rjq/KPGNN-main/MixData_0518/mixnonleatranfea.npy"
        # filename = './baselines/mbert_{}_features.npy'.format(lang)
        dfpath = "/data/rjq/KPGNN-main/MixData_0518/mixdf.npy"
    ...
    if args.lang == "Mix":
        ...
        nonleafilename = "./features_69612_0709_spacy_lg_zero_multiclasses_filtered_{}_{}.npy".format(args.lang, args.tgtlang)
        nf = np.load(nonleafilename)
        construct_incremental_dataset_0922(args, df, save_path, f, nf, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tgtlang', type=str, default='English')
    parser.add_argument('--days', type=int, default=121)
